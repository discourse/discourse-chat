...
class DiscourseChat::ChatMessageCreator
  ...
  def create
    begin
      ...
      mentioned_user_ids = DiscourseChat::ChatNotifier.notify_new(chat_message: @chat_message, timestamp: @chat_message.created_at)
      create_email_statuses(mentioned_user_ids)
    rescue => error
      @error = error
    end
  end
