...
function generateTranscriptHTML(messageContent, opts) {
  const channelDataAttr = opts.channel
    ...
    : "";
  const chainedDataAttr = opts.chained ? ` data-chained=\"true\"` : "";

  const transcript = [];
  transcript.push(
    `<div class=\"discourse-chat-transcript\" data-message-id=\"${opts.messageId}\" data-username=\"${opts.username}\" data-datetime=\"${opts.datetime}\"${chainedDataAttr}${channelDataAttr}>`
  );
