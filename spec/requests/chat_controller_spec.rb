...
RSpec.describe DiscourseChat::ChatController do
  ...
  describe "current_user_serializer" do
    ...
    fab!(:chat_channel_3) { Fabricate(:chat_channel, chatable: Fabricate(:topic)) }
    fab!(:user_chat_channel_membership1) { Fabricate(:user_chat_channel_membership, user: user, chat_channel: chat_channel_1) }
    fab!(:user_chat_channel_membership2) { Fabricate(:user_chat_channel_membership, user: user, chat_channel: chat_channel_2) }
    fab!(:user_chat_channel_membership3) { Fabricate(:user_chat_channel_membership, user: user, chat_channel: chat_channel_3) }
    ...
    describe "chat_channel_tracking_state" do
      it 'has correct unread_counts for each channel' do
        ...
        channel_3_state = payload[:chat_channel_tracking_state][chat_channel_3.id.to_s]
        expect(channel_1_state["unread_count"]).to eq(2)
        expect(channel_2_state["unread_count"]).to eq(1)
        expect(channel_3_state["unread_count"]).to eq(0)
      end
    end
  end
end
