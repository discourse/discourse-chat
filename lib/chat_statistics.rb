  ...
  def self.about_messages
    {
      ...
      previous_30_days: ChatMessage.where('created_at BETWEEN ? AND ?', 60.days.ago, 30.days.ago).count,
  ...
  def self.about_channels
    {
      ...
      previous_30_days: ChatChannel.where(status: :open).where('created_at BETWEEN ? AND ?', 60.days.ago, 30.days.ago).count,
  ...
  def self.about_users
    {
      ...
      previous_30_days: ChatMessage.where('created_at BETWEEN ? AND ?', 60.days.ago, 30.days.ago).distinct.count(:user_id),
